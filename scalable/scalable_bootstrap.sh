...
"{
    ...
    python3() {
        ...
        COMMUNICATOR_PID=\\\$!
        apptainer exec --userns --compat --env APPTAINER_VERSION=$APPTAINER_VERSION COMM_PORT=$COMM_PORT --env DASH_PORT=$DASH_PORT --home ~/$work_dir --cwd ~/$work_dir ~/$work_dir/containers/scalable_container.sif bash -i -c \\\"python3 \\\$@\\\"
